 ...
 * Time: 10:47
 */
class OpenFileDialog(context: Context) :
    ...
    private inner class FileAdapter(
        ...
        ArrayAdapter<File?>(context!!, R.layout.simple_list_item_1, files!!) {
        override fun getView(
            ...
            if (view != null) {
                ...
                if (file.isDirectory) {
                    ...
                    if (selectedIndex == position) view.setBackgroundColor(
                        context.resources.getColor(
                            R.color.holo_blue_dark
                        )
                    ) else view.setBackgroundColor(context.resources.getColor(R.color.transparent))
                }
            }
            return view
        }
    ...
    fun setFilter(filter: String?): OpenFileDialog {
        filenameFilter = FilenameFilter { file, fileName ->
            val tempFile =
                File(String.format("%s/%s", file.path, fileName))
    ...
    private fun getItemHeight(context: Context): Int {
        ...
        context.theme
            .resolveAttribute(R.attr.listPreferredItemHeightSmall, value, true)
    ...
    private fun createBackItem(context: Context): TextView {
        val textView =
            createTextView(context, R.style.TextAppearance_DeviceDefault_Small)
        val drawable =
            getContext().resources.getDrawable(R.drawable.ic_menu_directions)
        drawable.setBounds(0, 0, 60, 60)
        ...
        textView.setOnClickListener {
            ...
            if (parentDirectory != null) {
                ...
                RebuildFiles(listView.adapter as ArrayAdapter<File>)
            }
        }
        return textView
    }
    ...
    private fun getFiles(directoryPath: String): List<File> {
        ...
        Collections.sort(
            ...
            if (file.isDirectory && file2.isFile) -1 else if (file.isFile && file2.isDirectory) 1 else file.path
                .compareTo(file2.path)
        }
        return fileList
    }
    ...
    private fun RebuildFiles(adapter: ArrayAdapter<File>) {
        try {
            ...
            var message: String? =
                context.resources.getString(R.string.unknownName)
        ...
        listView.onItemClickListener =
            OnItemClickListener { adapterView, view, index, l ->
                val adapter: ArrayAdapter<File> =
                    adapterView.adapter as ArrayAdapter<File>
